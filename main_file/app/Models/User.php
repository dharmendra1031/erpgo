...
class User extends Authenticatable
{
    ...
    public function assignPlan($planID)
    {
        ...
        if($plan)
        {
            ...
            $users     = User::where('created_by', '=', $this->id)->where('type', '!=', 'super admin')->where('type', '!=', 'company')->where('type', '!=', 'client')->get();
            $clients   = User::where('created_by', '=', $this->id)->where('type', 'client')->get();
    ...
    {
        $revenue      = Revenue::where('created_by', '=', $this->creatorId())->whereDate('date', date('Y-m-d'))->where('created_by', \Auth::user()->creatorId())->sum('amount');
        $invoices     = Invoice:: select('*')->where('created_by', \Auth::user()->creatorId())->whereDate('send_date', date('Y-m-d'))->get();
        $invoiceArray = array();
    ...
    {
        $payment = Payment::where('created_by', '=', $this->creatorId())->where('created_by', \Auth::user()->creatorId())->whereDate('date', date('Y-m-d'))->sum('amount');

        $bills = Bill:: select('*')->where('created_by', \Auth::user()->creatorId())->whereDate('send_date', date('Y-m-d'))->get();
    ...
    public function getincExpBarChartData()
    {
        ...
        for($i = 1; $i <= 12; $i++)
        {
            $monthlyIncome = Revenue::selectRaw('sum(amount) amount')->where('created_by', '=', $this->creatorId())->whereYear('date', date('Y'))->whereMonth('date', $i)->first();
            $invoices      = Invoice:: select('*')->where('created_by', \Auth::user()->creatorId())->whereYear('send_date', date('Y'))->whereMonth('send_date', $i)->get();
            ...
            $bills          = Bill:: select('*')->where('created_by', \Auth::user()->creatorId())->whereYear('send_date', date('Y'))->whereMonth('send_date', $i)->get();
            $billArray      = array();
            foreach($bills as $bill)
            {
                $billArray[] = $bill->getTotal();
    ...
    {
        return ProjectTask::whereRaw(Utility::csvContainsExpression('assign_to'), [(string) $this->id])->get();
    }
