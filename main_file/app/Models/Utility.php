...
{
    /**
     * Return a bound, database-specific predicate for legacy CSV columns.
     *
     * The application historically stores a few ID/tag lists as comma-separated
     * strings. Keep those reads portable until the columns can be normalized
     * into pivot tables.
     */
    public static function csvContainsExpression($column)
    {
        if(DB::connection()->getDriverName() === 'sqlsrv')
        {
            return "EXISTS (SELECT 1 FROM STRING_SPLIT({$column}, ',') WHERE LTRIM(RTRIM(value)) = ?)";
        }
