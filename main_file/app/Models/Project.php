...
class Project extends Model
{
    ...
    public function countTask($user_id = 0)
    {
        ...
        {
            $usr           = $user_id;
            $complete_task = $this->tasks()->where('is_complete', '=', 1)->whereRaw(Utility::csvContainsExpression('assign_to'), [(string) $usr])->count();
            $total_task    = $this->tasks()->whereRaw(Utility::csvContainsExpression('assign_to'), [(string) $usr])->count();
        }
