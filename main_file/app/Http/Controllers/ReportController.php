...
class ReportController extends Controller
{
    public function incomeSummary(Request $request)
    {
        if(\Auth::user()->can('income report'))
        {
            ...
            $incomes->groupByRaw('MONTH(date), YEAR(date), category_id');
            $incomes = $incomes->get();
            ...
            $incomesData->groupByRaw('MONTH(date), YEAR(date)');
    ...
    public function expenseSummary(Request $request)
    {
        if(\Auth::user()->can('expense report'))
        {
            ...
            $expenses->groupByRaw('MONTH(date), YEAR(date), category_id');
            ...
            $expensesData->groupByRaw('MONTH(date), YEAR(date)');
    ...
    public function accountStatement(Request $request)
    {
        if(\Auth::user()->can('statement report'))
        {
            ...
            if($request->type == 'revenue' || !isset($request->type))
            {
                ...
                $revenueAccounts = Revenue::select('bank_accounts.id', 'bank_accounts.holder_name', 'bank_accounts.bank_name')->leftjoin('bank_accounts', 'revenues.account_id', '=', 'bank_accounts.id')->groupBy('revenues.account_id', 'bank_accounts.id', 'bank_accounts.holder_name', 'bank_accounts.bank_name')->selectRaw('sum(amount) as total');
    ...
    public function trialBalanceSummary(Request $request)
    {
        if(\Auth::user()->can('trial balance report'))
        {
            ...
            $journalItem->groupBy('account', 'chart_of_accounts.name');
