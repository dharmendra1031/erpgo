...
class TaxController extends Controller
{
    ...
    public function destroy(Tax $tax)
    {
        if(\Auth::user()->can('delete constant tax'))
        {
            if($tax->created_by == \Auth::user()->creatorId())
            {
                $proposalData = ProposalProduct::whereRaw(Utility::csvContainsExpression('tax'), [(string) $tax->id])->first();
                $billData     = BillProduct::whereRaw(Utility::csvContainsExpression('tax'), [(string) $tax->id])->first();
                $invoiceData  = InvoiceProduct::whereRaw(Utility::csvContainsExpression('tax'), [(string) $tax->id])->first();
