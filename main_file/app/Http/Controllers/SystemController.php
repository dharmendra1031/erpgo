...
class SystemController extends Controller
{
        ...
        if(\Auth::user()->can('manage company settings'))
        {
            ...
            foreach($post as $key => $data)
            {
                if(in_array($key, array_keys($settings)))
                {
                    \DB::table('settings')->updateOrInsert(
                        ['name' => $key, 'created_by' => \Auth::user()->creatorId()],
                        ['value' => $data, 'updated_at' => date('Y-m-d H:i:s')]
                    );
                }
            }
    ...
    public function saveZoomSettings(Request $request)
    {
        ...
        {
            \DB::table('settings')->updateOrInsert(
                ['name' => $key, 'created_by' => $created_by],
                ['value' => $data, 'updated_at' => date('Y-m-d H:i:s')]
            );
        }
    ...
    public function saveBusinessSettings(Request $request)
    {
        ...
        if(\Auth::user()->can('manage business settings'))
        {
            ...
            if($request->company_logo)
            {
                ...
                \DB::table('settings')->updateOrInsert(
                    ['name' => 'company_logo', 'created_by' => \Auth::user()->creatorId()],
                    ['value' => $logoName]
                );
            }
            ...
            if($request->company_small_logo)
            {
                ...
                \DB::table('settings')->updateOrInsert(
                    ['name' => 'company_small_logo', 'created_by' => \Auth::user()->creatorId()],
                    ['value' => $smallLogoName]
                );
            }
    ...
    public function adminPaymentSettings($request)
    {
        ...
        foreach($post as $key => $data)
        {
            ...
            \DB::table('admin_payment_settings')->updateOrInsert(
                ['name' => $key, 'created_by' => \Auth::user()->id],
                ['value' => $data]
            );
    ...
    public function saveSlackSettings(Request $request){
        ...
        if(isset($post) && !empty($post) && count($post) > 0)
        {
            ...
            foreach($post as $key => $data)
            {
                DB::table('settings')->updateOrInsert(
                    ['name' => $key, 'created_by' => Auth::user()->id],
                    ['value' => $data, 'updated_at' => $updated_at]
                );
            }
        }
