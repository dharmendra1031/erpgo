    ...
    public function allBugList($view)
    {
          ...
          elseif(Auth::user()->type != 'company'){
            if(\Auth::user()->type == 'client'){
              ...
              $bugs = Bug::where('created_by',\Auth::user()->creatorId())->whereRaw(Utility::csvContainsExpression('assign_to'), [(string) \Auth::user()->id])->get();
            }
          }
          if($view == 'list'){
        ...
        if($request->ajax() && $request->has('view') && $request->has('sort'))
        {
            ...
            if(\Auth::user()->type != 'company'){
              if(\Auth::user()->type == 'client'){
                ...
                $tasks->whereRaw(Utility::csvContainsExpression('assign_to'), [(string) $usr->id]);
              }
            }
            else{
              $tasks->where('created_by',\Auth::user()->creatorId());
        ...
        if($usr->type != 'admin')
        {
            ...
            elseif(\Auth::user()->type != 'company'){
              if(\Auth::user()->type == 'client'){
                ...
                $tasks = ProjectTask::whereIn('project_id', $user_projects)->whereRaw(Utility::csvContainsExpression('assign_to'), [(string) \Auth::user()->id]);
              }
            }
            if(\Auth::user()->type  == 'client'){
              if($task_by == 'all')
