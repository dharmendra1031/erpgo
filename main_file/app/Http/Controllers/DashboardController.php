...
class DashboardController extends Controller
{
    ...
    public function project_dashboard_index()
    {
        ...
        if(\Auth::user()->can('show project dashboard'))
        {
            ...
            else
            {
                ...
                // Total Tasks
                $complete_task           = ProjectTask::where('is_complete', '=', 1)->whereRaw(Utility::csvContainsExpression('assign_to'), [(string) $user->id])->whereIn('project_id', $user_projects)->count();
    ...
    public function filterView(Request $request)
    {
        ...
        if($request->ajax())
        {
            ...
            {
                $users->where('name', 'LIKE', $request->keyword . '%')->orWhereRaw(Utility::csvContainsExpression('skills'), [(string) $request->keyword]);
            }
