                            ...
                            @can('buy plan')
                                ...
                                @if($plan->id != 1 && $plan->id != \Auth::user()->plan)
                                    @if(\Auth::user()->requested_plan != $plan->id)
                                        <form method="POST" action="{{ route('send.request',[\Illuminate\Support\Facades\Crypt::encrypt($plan->id)])}}" class="d-inline">
                                            @csrf
                                            <button type="submit" class="button text-xs badge-success border-0" data-title="{{__('Send Request')}}" data-toggle="tooltip">
                                                <span class="btn-inner--icon"><i class="fas fa-share"></i></span>
                                            </button>
                                        </form>
                                    @else
                                        <form method="POST" action="{{ route('request.cancel') }}" class="d-inline">
                                            @csrf
                                            <button type="submit" class="button text-xs bg-danger border-0" data-title="{{__('Cancel Request')}}" data-toggle="tooltip">
